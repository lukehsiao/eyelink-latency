...
The circuit that is soldered to the Arduino Proto Shield is shown below. The RC
circuit preceding the photodiode serves as a low-pass filter. The load resister
is just a very large value in order to read a large voltage value from the small
amount of current generated by the photodiode.
